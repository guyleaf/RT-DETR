...
from ppdet.metrics import COCOMetric, Metric, VOCMetric, get_infer_results
...
class Trainer(object):
    def __init__(self, cfg, mode="train"):
        ...
        if self.use_amp and self.amp_level == "O2":
            self.model, self.optimizer = paddle.amp.decorate(
                models=self.model, optimizers=self.optimizer, level=self.amp_level
            )
    ...
    def _init_metrics(self, validate=False):
        ...
        if self.cfg.metric == "COCO":
            ...
            logger.warning(
                "Metric not support for metric type {}".format(self.cfg.metric)
            )
            self._metrics = []
    ...
    def resume_weights(self, weights):
        self.start_epoch = load_weight(
            self.model, weights, self.optimizer, self.ema if self.use_ema else None
        )
    ...
    def train(self, validate=False):
        ...
        sync_bn = (
            getattr(self.cfg, "norm_type", None) == "sync_bn"
            and (self.cfg.use_gpu or self.cfg.use_mlu)
            and self._nranks > 1
        )
        if sync_bn:
        ...
        if self.cfg.get("fleet", False):
            ...
            find_unused_parameters = (
                self.cfg["find_unused_parameters"]
                if "find_unused_parameters" in self.cfg
                else False
            )
        ...
        use_fused_allreduce_gradients = (
            self.cfg["use_fused_allreduce_gradients"]
            if "use_fused_allreduce_gradients" in self.cfg
            else False
        )
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                ...
                if self.use_amp:
                    if (
                        ...
                        and use_fused_allreduce_gradients
                    ):
                        with model.no_sync():
            ...
            is_snapshot = (
                self._nranks < 2
                or (self._local_rank == 0 or self.cfg.metric == "Pose3DEval")
            ) and (
                (epoch_id + 1) % self.cfg.snapshot_epoch == 0
            ...
            if validate and is_snapshot:
                if not hasattr(self, "_eval_loader"):
                    ...
                    self._eval_batch_sampler = paddle.io.BatchSampler(
                        self._eval_dataset, batch_size=self.cfg.EvalReader["batch_size"]
                    )
                    # If metric is VOC, need to be set collate_batch=False.
    ...
    def slice_predict(
        ...
        def setup_metrics_for_loader():
            ...
            save_prediction_only = (
                self.cfg["save_prediction_only"]
                if "save_prediction_only" in self.cfg
                else None
            )
            output_eval = self.cfg["output_eval"] if "output_eval" in self.cfg else None
        ...
        if visualize:
            for outs in results:
                ...
                for i, im_id in enumerate(outs["im_id"]):
                    ...
                    bbox_res = (
                        batch_res["bbox"][start:end] if "bbox" in batch_res else None
                    )
                    mask_res = (
                        batch_res["mask"][start:end] if "mask" in batch_res else None
                    )
                    segm_res = (
                        batch_res["segm"][start:end] if "segm" in batch_res else None
                    )
                    keypoint_res = (
                        batch_res["keypoint"][start:end]
                        if "keypoint" in batch_res
                        else None
                    )
                    pose3d_res = (
                        batch_res["pose3d"][start:end]
                        if "pose3d" in batch_res
                        else None
                    )
    ...
    def _get_infer_cfg_and_input_spec(self, save_dir, prune_input=True, kl_quant=False):
        image_shape = None
        im_shape = [None, 2]
        scale_factor = [None, 2]
        ...
        if (
            hasattr(self.cfg, "export")
            and "fuse_conv_bn" in self.cfg["export"]
            and self.cfg["export"]["fuse_conv_bn"]
        ):
            self.model = fuse_conv_bn(self.model)

        export_post_process = (
            self.cfg["export"].get("post_process", False)
            if hasattr(self.cfg, "export")
            else True
        )
        export_nms = (
            self.cfg["export"].get("nms", False)
            if hasattr(self.cfg, "export")
            else True
        )
        export_benchmark = (
            self.cfg["export"].get("benchmark", False)
            if hasattr(self.cfg, "export")
            else False
        )
        if hasattr(self.model, "export_post_process"):
            self.model.export_post_process = (
                export_post_process if not export_benchmark else False
            )
        ...
        # Save infer cfg
        _dump_infer_config(
            self.cfg, os.path.join(save_dir, "infer_cfg.yml"), image_shape, self.model
        )

        input_spec = [
            {
                "image": InputSpec(shape=image_shape, name="image"),
    ...
    def export(self, output_dir="output_inference"):
        ...
        static_model, pruned_input_spec = self._get_infer_cfg_and_input_spec(save_dir)

        # dy2st and save model
        paddle.jit.save(
            static_model, os.path.join(save_dir, "model"), input_spec=pruned_input_spec
        )
