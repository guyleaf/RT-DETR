    ...
    "tinypose_128x96": [
        {
            ...
            "score": paddle.static.InputSpec(name="score", shape=[-1], dtype="float32"),
            ...
            for key, value in bt.items():
                ...
                if key == "PadBatch":
                    preprocess_list.append(
                        {"type": "PadStride", "stride": value["pad_to_stride"]}
                    )
                    break
...
def _dump_infer_config(config, path, image_shape, model):
    ...
    if "RCNN" in infer_arch and export_onnx:
        logger.warning("Exporting RCNN model to ONNX only support batch_size = 1")
    ...
    if not arch_state:
        ...
        os._exit(0)
    if (
        "mask_head" in config[config["architecture"]]
        and config[config["architecture"]]["mask_head"]
    ):
        infer_cfg["mask"] = True
    ...
    if infer_arch == "PicoDet":
        if (
            hasattr(config, "export")
            and config["export"].get("post_process", False)
            and not config["export"].get("benchmark", False)
        ):
            infer_cfg["arch"] = "GFL"
