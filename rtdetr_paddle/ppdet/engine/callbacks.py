...
class ComposeCallback(object):
    def __init__(self, callbacks):
        ...
        for c in callbacks:
            assert isinstance(c, Callback), "callback should be subclass of Callback"
    ...
    def on_epoch_end(self, status):
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if mode == "eval":
                ...
                logger.info(
                    "Total sample number: {}, average FPS: {}".format(
        ...
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            if mode == "train":
                ...
                if (
                    ...
                    save_name = (
                        str(epoch_id) if epoch_id != end_epoch - 1 else "model_final"
                    )
            ...
            if mode == "train":
                ...
                for loss_name, loss_value in training_status.get().items():
                    self.vdl_writer.add_scalar(
                        loss_name, loss_value, self.vdl_loss_step
                    )
            ...
            if mode == "eval":
                for metric in self.model._metrics:
                    for key, map_value in metric.get_results().items():
                        self.vdl_writer.add_scalar(
                            "{}-mAP".format(key), map_value[0], self.vdl_mAP_step
                        )
                self.vdl_mAP_step += 1
        ...
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if ema_model is None:
                ...
                model_artifact = self.wandb.Artifact(
                    name="model-{}".format(self.run.id), type="model", metadata=metadata
                )
