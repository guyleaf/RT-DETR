...
class COCODataSet(DetDataset):
    ...
    def __init__(
        ...
        super(COCODataSet, self).__init__(
            dataset_dir, image_dir, anno_path, data_fields, sample_num, repeat=repeat
        )
    ...
    def parse_dataset(self):
        ...
        for img_id in img_ids:
            ...
            if not os.path.exists(im_path):
                logger.warning(
                    "Illegal image file: {}, and it will be ignored".format(im_path)
                )
                continue
...
class SlicedCOCODataSet(COCODataSet):
    ...
    def parse_dataset(self):
        ...
        assert ct > 0, "not found any coco record in %s" % (anno_path)
        logger.info(
            "{} samples and slice to {} sub_samples in file {}".format(
                ct, ct_sub, anno_path
            )
        )
        if self.allow_empty and len(empty_records) > 0:
    ...
    def parse_dataset(self):
        ...
        if self.supervised:
            ...
            if self.length > 0:  # unsup length will be decide by sup length
                ...
                selected_idxs = [
                    np.random.choice(len(all_roidbs)) for _ in range(self.length)
                ]
                self.roidbs = [all_roidbs[i] for i in selected_idxs]
            logger.info(f"Use {len(self.roidbs)} unsup_samples data as UNLABELED")
