...
level_start_index = paddle.concat(
    (paddle.to_tensor([0], dtype=paddle.int64), spatial_shapes.prod(1).cumsum(0)[:-1])
)
...
def get_test_tensors(channels):
    value = (
        paddle.rand([bs, value_length, n_heads, channels], dtype=paddle.float32) * 0.01
    )
    sampling_locations = paddle.rand(
        [bs, query_length, n_heads, n_levels, n_points, 2], dtype=paddle.float32
    )
    attention_weights = (
        paddle.rand(
...
def check_gradient_numerical(channels=4):
    value_paddle, sampling_locations_paddle, attention_weights_paddle = (
    ...
    res = paddle.allclose(
        value_paddle.grad, value_cuda.grad, rtol=1e-2, atol=1e-3
    ).item()
    ...
    res = paddle.allclose(
        attention_weights_paddle.grad, attention_weights_cuda.grad, rtol=1e-2, atol=1e-3
    ).item()
