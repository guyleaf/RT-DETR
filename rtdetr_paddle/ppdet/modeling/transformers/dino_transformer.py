...
class DINOTransformerDecoder(nn.Layer):
    def __init__(
        self, hidden_dim, decoder_layer, num_layers, weight_attr=None, bias_attr=None
    ):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            reference_points_input = ref_points.detach().unsqueeze(
                2
            ) * valid_ratios.tile([1, 1, 2]).unsqueeze(1)
            ...
            ref_points = F.sigmoid(
                bbox_head[i](output) + inverse_sigmoid(ref_points.detach())
            )
...
class DINOTransformer(nn.Layer):
    ...
    def __init__(
        ...
        self.decoder = DINOTransformerDecoder(
            hidden_dim, decoder_layer, num_decoder_layers, weight_attr, bias_attr
        )
        ...
        self.query_pos_head = MLP(2 * hidden_dim, hidden_dim, hidden_dim, num_layers=2)

        # encoder head
        self.enc_output = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.LayerNorm(hidden_dim, weight_attr=weight_attr, bias_attr=bias_attr),
        )
    ...
    def _build_input_proj_layer(
        self, in_feats_channel, weight_attr=None, bias_attr=None
    ):
        self.input_proj = nn.LayerList()
        for in_channels in in_feats_channel:
            self.input_proj.append(
                nn.Sequential(
                    ("conv", nn.Conv2D(in_channels, self.hidden_dim, kernel_size=1)),
                    (
                        "norm",
                        nn.GroupNorm(
                            32,
    ...
    def _get_encoder_input(self, feats, pad_mask=None):
        ...
        # [b, l]
        mask_flatten = None if pad_mask is None else paddle.concat(mask_flatten, 1)
        # [b, l, c]
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None):
        ...
        (
            ...
            lvl_pos_embed_flatten,
            valid_ratios,
        ) = self._get_encoder_input(feats, pad_mask)
        ...
        for i in range(self.num_decoder_layers):
            ...
            if i == 0:
                out_bboxes.append(
                    F.sigmoid(
                        self.dec_bbox_head[i](inter_feats[i]) + init_ref_points_unact
                    )
                )
            else:
                out_bboxes.append(
                    F.sigmoid(
                        self.dec_bbox_head[i](inter_feats[i])
    ...
    def _get_encoder_output_anchors(
        self, memory, spatial_shapes, memory_mask=None, grid_size=0.05
    ):
        output_anchors = []
        idx = 0
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            grid_y, grid_x = paddle.meshgrid(paddle.arange(end=h), paddle.arange(end=w))
            grid_xy = paddle.stack([grid_x, grid_y], -1).astype(memory.dtype)

            valid_WH = (
                paddle.stack([valid_W, valid_H], -1)
                .reshape([-1, 1, 1, 2])
                .astype(grid_xy.dtype)
            )
        ...
        valid_mask = (
            (output_anchors > self.eps) * (output_anchors < 1 - self.eps)
        ).all(-1, keepdim=True)
        output_anchors = paddle.log(output_anchors / (1 - output_anchors))
        if memory_mask is not None:
            valid_mask = (valid_mask * (memory_mask.unsqueeze(-1) > 0)) > 0
        output_anchors = paddle.where(
            valid_mask, output_anchors, paddle.to_tensor(float("inf"))
        )
    ...
    def _get_decoder_input(
        ...
        enc_outputs_coord_unact = self.enc_bbox_head(output_memory) + output_anchors
