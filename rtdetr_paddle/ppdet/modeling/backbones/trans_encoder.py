...
from paddle.nn import GELU, ReLU, Swish
...
class BertEmbeddings(nn.Layer):
    def __init__(
        ...
        self.word_embeddings = nn.Embedding(word_size, hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(position_embeddings_size, hidden_size)
...
class BertSelfAttention(nn.Layer):
    def __init__(
        ...
        if hidden_size % num_attention_heads != 0:
            raise ValueError(
                ...
                "heads, but got {} % {} != 0" % (hidden_size, num_attention_heads)
            )
    ...
    def forward(self, x, attention_mask, head_mask=None):
        ...
        attention = paddle.matmul(query, key) / math.sqrt(self.attention_head_size)
        ...
        context = paddle.matmul(attention_value, value).transpose(perm=(0, 2, 1, 3))
...
class METROEncoder(nn.Layer):
    def __init__(
        ...
        self.embedding = BertEmbeddings(
            vocab_size, position_embeddings_size, 2, hidden_size, fc_dropout_prob
        )
        ...
        self.position_embeddings = nn.Embedding(position_embeddings_size, hidden_size)
        self.img_embedding = nn.Linear(features_dims, hidden_size, bias_attr=True)
        self.dropout = nn.Dropout(fc_dropout_prob)
    ...
    def init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            module.weight.set_value(
                paddle.normal(mean=0.0, std=0.02, shape=module.weight.shape)
            )
        elif isinstance(module, nn.LayerNorm):
            module.bias.set_value(paddle.zeros(shape=module.bias.shape))
            module.weight.set_value(
                paddle.full(shape=module.weight.shape, fill_value=1.0)
            )
    ...
    def forward(self, x):
        ...
        position_ids = (
            paddle.arange(seq_len, dtype="int64").unsqueeze(0).expand_as(input_ids)
        )
        ...
        encoder_outputs = self.encoder(embeddings, attention_mask, head_mask=head_mask)
