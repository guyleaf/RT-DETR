    ...
    def get_rel_pos(self, seq_size, rel_pos):
        ...
        if rel_pos.shape[0] != max_rel_dist:
            ...
            rel_pos_resized = F.interpolate(
                rel_pos, size=(max_rel_dist,), mode="linear", data_format="NCW"
            )
...
class Block(nn.Layer):
    def __init__(
        ...
        self.norm1 = eval(norm_layer)(
            dim,
            weight_attr=ParamAttr(learning_rate=lr_factor, regularizer=L2Decay(0.0)),
            bias_attr=ParamAttr(learning_rate=lr_factor, regularizer=L2Decay(0.0)),
            epsilon=epsilon,
        )
        self.attn = Attention(
            dim,
...
class VisionTransformer2D(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_abs_pos_emb:
            ...
            pos_embed = self.get_2d_sincos_position_embedding(
        ...
        self.blocks = nn.LayerList(
            [
                Block(
                    ...
                    use_rel_pos=use_rel_pos,
                    rel_pos_zero_init=rel_pos_zero_init,
                    window_size=None if i in self.global_attn_indexes else window_size,
        ...
        self.out_strides = (
            [4, 8, 16, 32][-len(out_indices) :]
            if with_fpn
            else [patch_size for _ in range(len(out_indices))]
        )
        self.norm = Identity()
        if self.with_fpn:
            self.init_fpn(
                embed_dim=embed_dim, patch_size=patch_size, out_with_norm=final_norm
            )
        ...
        if pretrained:
            ...
            if pos_embed_name in load_state_dict.keys() and self.use_abs_pos_emb:
                ...
                if self.pos_embed.shape != load_pos_embed.shape:
                    ...
                    load_state_dict[pos_embed_name] = model_state_dict[pos_embed_name]
    ...
    def get_2d_sincos_position_embedding(self, h, w, temperature=10000.0):
        grid_y, grid_x = paddle.meshgrid(
            paddle.arange(h, dtype=paddle.float32),
            paddle.arange(w, dtype=paddle.float32),
        )
        ...
        omega = (1.0 / (temperature**omega)).unsqueeze(0)
    ...
    def forward(self, inputs):
        ...
        if self.use_abs_pos_emb:
            x = x + self.resize_pos_embed(
                self.pos_embed, (self.pos_h, self.pos_w), (Hp, Wp)
            )
...
class SimpleFeaturePyramid(nn.Layer):
    def __init__(
        self, in_channels, out_channels, spatial_scales, num_levels=4, use_bias=False
    ):
        """
        Args:
