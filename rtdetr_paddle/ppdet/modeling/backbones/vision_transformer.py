...
from .transformer_utils import DropPath, Identity, zeros_
...
class Attention(nn.Layer):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        qkv = qkv.reshape((-1, N, 3, self.num_heads, C // self.num_heads)).transpose(
            (2, 0, 3, 1, 4)
        )
...
class RelativePositionBias(nn.Layer):
    def __init__(self, window_size, num_heads):
        ...
        self.relative_position_bias_table = self.create_parameter(
            shape=(self.num_relative_distance, num_heads), default_initialize=zeros_
        )
        # cls to token & token 2 cls & cls to cls
    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.out_indices:
                xp = paddle.reshape(
                    paddle.transpose(self.norm(x[:, 1:, :]), perm=[0, 2, 1]),
                    shape=[B, D, Hp, Wp],
                )
                feats.append(xp)

        if self.with_fpn:
            fpns = [self.fpn1, self.fpn2, self.fpn3, self.fpn4][-self.num_fpn_levels :]
            assert len(fpns) == len(feats) or len(feats) == 1, ""
            outputs = []
            for i, m in enumerate(fpns):
                outputs.append(m(feats[i] if len(feats) == len(fpns) else feats[-1]))
