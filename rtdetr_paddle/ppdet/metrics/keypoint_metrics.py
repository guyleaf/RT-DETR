...
class KeyPointTopDownCOCOEval(object):
    ...
    def update(self, inputs, outputs):
        ...
        self.results["all_preds"][self.idx : self.idx + num_images, :, 0:3] = kpts[
            :, :, 0:3
        ]
        self.results["all_boxes"][self.idx : self.idx + num_images, 0:2] = (
            inputs["center"].numpy()[:, 0:2]
            if isinstance(inputs["center"], paddle.Tensor)
            else inputs["center"][:, 0:2]
        )
        self.results["all_boxes"][self.idx : self.idx + num_images, 2:4] = (
            inputs["scale"].numpy()[:, 0:2]
            if isinstance(inputs["scale"], paddle.Tensor)
            else inputs["scale"][:, 0:2]
        )
        self.results["all_boxes"][self.idx : self.idx + num_images, 4] = (
            np.prod(inputs["scale"].numpy() * 200, 1)
            if isinstance(inputs["scale"], paddle.Tensor)
            else np.prod(inputs["scale"] * 200, 1)
        )
        self.results["all_boxes"][self.idx : self.idx + num_images, 5] = (
            np.squeeze(inputs["score"].numpy())
            if isinstance(inputs["score"], paddle.Tensor)
            else np.squeeze(inputs["score"])
        )
    ...
    def accumulate(self):
        ...
        if self.save_prediction_only:
            logger.info(
                f"The keypoint result is saved to {self.res_file} "
...
class KeyPointTopDownMPIIEval(object):
    ...
    def evaluate(self, outputs, savepath=None):
        ...
        gt_file = os.path.join(os.path.dirname(self.ann_file), "mpii_gt_val.mat")
