...
class COCOMetric(Metric):
    def __init__(self, anno_file, **kwargs):
        ...
        if not self.save_prediction_only:
            assert os.path.isfile(anno_file), "anno_file {} not a file".format(
                anno_file
            )
    ...
    def update(self, inputs, outputs):
        ...
        self.results["mask"] += infer_results["mask"] if "mask" in infer_results else []
        self.results["segm"] += infer_results["segm"] if "segm" in infer_results else []
        self.results["keypoint"] += (
            infer_results["keypoint"] if "keypoint" in infer_results else []
        )
    ...
    def accumulate(self):
        ...
        if len(self.results["keypoint"]) > 0:
            ...
            if self.save_prediction_only:
                logger.info(
                    "The keypoint result is saved to {} and do not "
                    "evaluate the mAP.".format(output)
                )
            else:
                style = "keypoints"
...
class VOCMetric(Metric):
    def __init__(
        ...
        assert os.path.isfile(label_list), "label_list {} not a file".format(label_list)
    ...
    def update(self, inputs, outputs):
        bbox_np = (
            outputs["bbox"].numpy()
            if isinstance(outputs["bbox"], paddle.Tensor)
            else outputs["bbox"]
        )
        bboxes = bbox_np[:, 2:]
        scores = bbox_np[:, 1]
        labels = bbox_np[:, 0]
        bbox_lengths = (
            outputs["bbox_num"].numpy()
            if isinstance(outputs["bbox_num"], paddle.Tensor)
            else outputs["bbox_num"]
        )
        ...
        difficults = inputs["difficult"] if not self.evaluate_difficult else None

        if "scale_factor" in inputs:
            scale_factor = (
                inputs["scale_factor"].numpy()
                if isinstance(inputs["scale_factor"], paddle.Tensor)
                else inputs["scale_factor"]
            )
        else:
        ...
        for i in range(len(gt_boxes)):
            ...
            gt_label = (
                gt_labels[i].numpy()
                if isinstance(gt_labels[i], paddle.Tensor)
                else gt_labels[i]
            )
            ...
            self.detection_map.update(bbox, score, label, gt_box, gt_label, difficult)
            bbox_idx += bbox_num
    ...
    def log(self):
        map_stat = 100.0 * self.detection_map.get_map()
        logger.info(
            "mAP({:.2f}, {}) = {:.2f}%".format(
            ...
            for image_id, image_path in imid2path.items():
                ...
                with open(output, "w") as f:
                    for det in dets:
                        ...
                        bbox_pred = "{} {} ".format(
                            self.catid2name[catid], score
                        ) + " ".join([str(e) for e in bbox])
                        f.write(bbox_pred + "\n")
...
class SNIPERCOCOMetric(COCOMetric):
    ...
    def accumulate(self):
        ...
        for outs in results:
            infer_results = get_infer_results(outs, self.clsid2catid, bias=self.bias)
            self.results["bbox"] += (
                infer_results["bbox"] if "bbox" in infer_results else []
            )
