...
def load_weight(model, weight, optimizer=None, ema=None, exchange=True):
    ...
    if ema is not None and os.path.exists(path + ".pdema"):
        if exchange:
            ...
            logger.info("Loading ema_model weights from {}".format(path + ".pdparams"))
            ...
            logger.info("Loading ema_model weights from {}".format(path + ".pdema"))
            param_state_dict = paddle.load(pdparam_path)
            logger.info("Loading model weights from {}".format(path + ".pdparams"))
    else:
        ema_state_dict = None
        param_state_dict = paddle.load(pdparam_path)
    ...
    if hasattr(model, "modelTeacher") and hasattr(model, "modelStudent"):
        ...
        for key in model_dict.keys():
            if key in param_state_dict.keys():
                ...
                incorrect_keys += 1
        assert incorrect_keys == 0, (
            "Load weight {} incorrectly, \
                {} keys unmatched, please check again.".format(weight, incorrect_keys)
        )
...
def match_state_dict(model_state_dict, weight_state_dict, mode="default"):
    ...
    if weight_keys[0].startswith("modelStudent") or weight_keys[0].startswith(
        ...
        if len(not_load_weight_name) > 0:
            logger.info(
                "{} in model is not matched with pretrained weights, "
                "and its will be trained from scratch".format(not_load_weight_name)
            )
    ...
    for model_id, weight_id in enumerate(max_id):
        ...
        if weight_key in matched_keys:
            raise ValueError(
                "Ambiguity weight {} loaded, it matches at least "
                "{} and {} in the model".format(
                    weight_key, model_key, matched_keys[weight_key]
                )
            )
...
def save_model(model, optimizer, save_dir, save_name, last_epoch, ema_model=None):
    """
    ...
    if isinstance(model, nn.Layer):
        ...
        assert isinstance(model, dict), "model is not a instance of nn.layer or dict"
        if ema_model is None:
            paddle.save(model, save_path + ".pdparams")
        else:
            assert isinstance(ema_model, dict), (
