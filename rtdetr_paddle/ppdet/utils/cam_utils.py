...
def compute_ious(boxes1, boxes2):
    ...
    intersection_area = (
        intersection_wh[:, :, 0] * intersection_wh[:, :, 1]
    )  # with shape (N,M)
...
class BBoxCAM:
    ...
    def get_bboxes_cams(self):
        ...
        if self.cfg.architecture in self.nms_idx_need_divid_numclass_arch:
            ...
            before_nms_indexes = (
                extra_data["nms_keep_idx"].cpu().numpy() // self.num_class
            )  # num_class
        else:
        ...
        for index, target_bbox in enumerate(inference_result["bbox"]):
            ...
            if (
                "backbone" in self.target_layer_name or "neck" in self.target_layer_name
            ):  # backbone/neck level feature
                if isinstance(self.target_feats[self.target_layer_name], list):
                    ...
                    if self.target_feats[self.target_layer_name][-1].shape[-1] == 1:
                        ...
                        """
                        cam_grad = (
                            self.target_feats[self.target_layer_name][-2]
                            .grad.squeeze()
                            .cpu()
                            .numpy()
                        )
                        ...
                        cam_grad = (
                            self.target_feats[self.target_layer_name][-1]
                            .grad.squeeze()
                            .cpu()
                            .numpy()
                        )
                    ...
                    cam_grad = (
                        self.target_feats[self.target_layer_name]
                        .grad.squeeze()
                        .cpu()
                        .numpy()
                    )
            ...
            if "backbone" in self.target_layer_name or "neck" in self.target_layer_name:
                ...
                mask[
                    int(target_bbox[3]) : int(target_bbox[5]),
            ...
            cv2.rectangle(
                ...
                (int(target_bbox[4]), int(target_bbox[5])),
                (0, 0, 255),
                2,
            )
