...
def get_config_path(url):
    ...
    cfg_url = (
        "ppdet://configs/{}/configs.tar".format(version)
        if version
        else "ppdet://configs/configs.tar"
    )
...
def get_dataset_path(path, annotation, image_dir):
    ...
    if data_name not in DOWNLOAD_DATASETS_LIST:
        raise ValueError(
            "Dataset {} is not valid for reason above, please check again.".format(
                osp.realpath(path)
            )
        )
    else:
    ...
    for name, dataset in DATASETS.items():
        if data_name == name:
            logger.debug("Parse dataset_dir {} as dataset {}".format(path, name))
...
def get_path(url, root_dir, md5sum=None, check_exist=True):
    ...
    if osp.exists(fullpath) and check_exist:
        if not osp.isfile(fullpath) or _check_exist_file_md5(fullpath, md5sum, url):
...
def download_dataset(path, dataset=None):
    if dataset not in DATASETS.keys():
        logger.error(
            "Unknown dataset {}, it should be {}".format(dataset, DATASETS.keys())
        )
        return
...
def _dataset_exists(path, annotation, image_dir):
    ...
    if not osp.exists(path):
        logger.warning(
            "Config dataset_dir {} is not exits, dataset config is not valid".format(
                path
            )
        )
        return False
...
def _download(url, path, md5sum=None):
    ...
    while not (osp.exists(fullname) and _check_exist_file_md5(fullname, md5sum, url)):
...
def _check_exist_file_md5(filename, md5sum, url):
    ...
    return (
        _md5check_from_url(filename, url)
        if md5sum is None and filename.endswith("pdparams")
...
def _md5check_from_url(filename, url):
    ...
    if not content_md5 or _md5check(
        filename, binascii.hexlify(base64.b64decode(content_md5.strip('"'))).decode()
    ):
        return True
    else:
        return False
