...
def draw_bbox(image, im_id, catid2name, bboxes, threshold):
    ...
    for dt in np.array(bboxes):
        ...
        if len(bbox) == 4:
            ...
            draw.line(
                [(x1, y1), (x2, y2), (x3, y3), (x4, y4), (x1, y1)], width=2, fill=color
            )
...
def draw_segm(image, im_id, catid2name, segms, threshold, alpha=0.7, draw_box=True):
    """
...
def draw_pose(
    ...
    for i in range(NUM_EDGES):
        for j in range(len(skeletons)):
            edge = EDGES[i]
            if (
                skeletons[j][edge[0], 2] < visual_thread
                or skeletons[j][edge[1], 2] < visual_thread
            ):
                continue
...
def draw_pose3d(
    ...
    if pose3d.shape[0] == 24:
        ...
        print(
            "not defined joints number :{}, cannot visualize because unknown of joint connectivity".format(
                pose.shape[0]
            )
        )
        return

    def draw3Dpose(pose3d, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
        #    pose3d = orthographic_projection(pose3d, cam)
        for i in joints_connectivity_dict:
            x, y, z = [np.array([pose3d[i[0], j], pose3d[i[1], j]]) for j in range(3)]
            ax.plot(-x, -z, -y, lw=2, c=lcolor if i[2] else rcolor)

        RADIUS = 1000
        center_xy = 2 if pose3d.shape[0] == 14 else 14
        x, y, z = pose3d[center_xy, 0], pose3d[center_xy, 1], pose3d[center_xy, 2]
    ...
    def draw2Dpose(pose2d, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
        for i in joints_connectivity_dict:
            if pose2d[i[0], 2] and pose2d[i[1], 2]:
                x, y = [np.array([pose2d[i[0], j], pose2d[i[1], j]]) for j in range(2)]
                ax.plot(x, y, 0, lw=2, c=lcolor if i[2] else rcolor)

    def draw_img_pose(pose3d, pose2d=None, frame=None, figsize=(12, 12), savepath=None):
