                    ...
                    if p_type == "rectangle":
                        ...
                        annotations_list.append(
                            annotations_rectangle(
                                points, label, image_num, object_num, label_to_num
                            )
                        )
            elif ds_type == "cityscape":
...
def widerface_to_cocojson(root_path):
    train_gt_txt = os.path.join(
        root_path, "wider_face_split", "wider_face_train_bbx_gt.txt"
    )
    val_gt_txt = os.path.join(
        root_path, "wider_face_split", "wider_face_val_bbx_gt.txt"
    )
    ...
    print(
        "Wider Face train dataset converts sucess, the json path: {}".format(save_path)
    )
...
def widerface_convert(gt_txt, img_dir, save_path):
    ...
    while i < len(lines):
        ...
        if img_info:
            ...
            print(
                "The image dose not exist: {}".format(os.path.join(img_dir, image_name))
            )
    ...
    if args.dataset_type == "voc":
        ...
        if args.train_proportion != 0:
            ...
            train_json_path = osp.join(
                args.output_dir + "/annotations", "instance_train.json"
            )
            json.dump(
                train_data_coco, open(train_json_path, "w"), indent=4, cls=MyEncoder
            )
        if args.val_proportion != 0:
            val_data_coco = deal_json(
                args.dataset_type, args.output_dir + "/val", args.json_input_dir
            )
            val_json_path = osp.join(
                args.output_dir + "/annotations", "instance_val.json"
            )
            json.dump(val_data_coco, open(val_json_path, "w"), indent=4, cls=MyEncoder)
        if args.test_proportion != 0:
            test_data_coco = deal_json(
                args.dataset_type, args.output_dir + "/test", args.json_input_dir
            )
            test_json_path = osp.join(
                args.output_dir + "/annotations", "instance_test.json"
            )
            json.dump(
